...
from src.ppo import PPOAgent, PPOAgentICM, PPOAgentRND
...
class AgentFactory:
    ...
    def _make_buffer(self) -> TensorRolloutBuffer:
        store_next = (self.config.agent == "ppo_icm") or (self.config.agent == "ppo_rnd")
    ...
    def _make_ppo_icm_agent(
        ...
        ppo_agent = PPOAgentICM(
            ...
            intrinsic_reward_eta=self.config.intrinsic_reward_eta,
    ...
    def _make_ppo_rnd_agent(
        self, buffer: TensorRolloutBuffer, feature_extractor: nn.Module
    ) -> PPOAgentICM:
        assert isinstance(self.config.intrinsic_reward_eta, float) and isinstance(
            self.config.lr_rnd, float
        ), "Missing required args!"
        assert buffer.next_states is not None, "Buffer next_state must be not None in ICM."

        ppo_agent = PPOAgentRND(
            obs_dim=self.obs_dim,
            action_dim=self.action_dim,
            hidden_dim=self.config.hidden_dim,
            lr_actor=self.config.lr_actor,
    ...
    def make_agent(self) -> PPOAgent:
        ...
        elif self.config.agent.lower() == "ppo_rnd":
            agent = self._make_ppo_rnd_agent(buffer, feature_extractor)
        else:
